            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 flex-1">
                    ...
                    <input wire:model.live="search"
                           ...
                           class="w-full sm:flex-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-sm transition-colors">
                </div>
                ...
                @if($hasItems && $items)
                    <div class="text-xs sm:text-sm text-gray-500 order-first sm:order-last">
                        <span class="inline-block bg-gray-100 px-2 py-1 rounded-md">
                            {{ $items->count() }} of {{ $items->total() }} drinks
                        </span>
                    </div>
                @endif
            </div>

            <!-- Drinks Search Loader -->
            <div wire:loading wire:target="search" class="flex justify-center mt-4">
                <div class="flex items-center gap-2 px-4 py-2 rounded-full bg-blue-100 text-blue-700 text-sm font-semibold shadow-sm animate-bounce transition-all">
                    <svg class="h-5 w-5 text-blue-500 animate-pulse" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div wire:loading.remove>
                                ...
                                <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden border group">
                                    <div class="relative overflow-hidden bg-gray-100">
                                        @if($item->image)
                                            <img src="{{ $item->image }}"
                                                 ...
                                                 loading="lazy">
                                        @else
                                            <div class="w-full h-40 sm:h-48 bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center">
                                    ...
                                    <div class="p-4 sm:p-5">
                                        ...
                                        <p class="text-xs sm:text-sm text-gray-600 mb-4 leading-relaxed h-10 overflow-hidden">
                                            {{ $item->description ?? 'Refreshing drink perfect for any occasion.' }}
                                        </p>
            ...
            @else
                <div class="bg-white rounded-lg shadow-sm border p-8 sm:p-12 text-center">
                    ...
                    <h3 class="text-lg sm:text-xl font-semibold text-gray-600 mb-2">No drinks found</h3>
                    <p class="text-gray-500 text-sm sm:text-base max-w-md mx-auto mb-4">
                        @if($search)
                            No drinks found matching "{{ $search }}". Try a different search term.
                        @else
                            Our refreshing drinks are currently being prepared. Check back soon!
                        @endif
                    </p>
                    @if($search)
                        <button wire:click="$set('search', '')"
                                class="bg-blue-500 hover:bg-blue-800 text-white px-6 py-2 rounded-md transition-colors duration-200 text-sm font-medium">
                            Clear Search
                        </button>
                    @endif
                </div>
