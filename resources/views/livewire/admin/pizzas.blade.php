<div class="p-3 sm:p-4 lg:p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
    ...
    <div class="mb-4 sm:mb-6">
        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-1 sm:mb-2">🍕 Pizza Management</h1>
        <p class="text-gray-600 text-xs sm:text-sm lg:text-base">Manage and track all pizzas</p>
    </div>
        ...
        <div class="p-3 sm:p-4">
            <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 items-end">
                <div class="flex-1 w-full">
                    <label for="search" class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Search Pizzas</label>
                    <div class="relative">
                ...
                @if($editingPizzaId === $pizza->id)
                    ...
                    <div class="p-4 sm:p-5 space-y-4 bg-gradient-to-br from-blue-50 to-indigo-50">
                        <div class="flex items-center space-x-2 mb-4">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900">Edit Pizza</h3>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Pizza Image</label>
                                @if($currentImage)
                                    <div class="mb-3">
                    ...
                    <div class="flex">
                        ...
                        <div class="flex-1 p-3 sm:p-4">
                            <div class="flex justify-between items-start mb-2">
                                <h3 class="text-base sm:text-lg font-semibold text-gray-900 leading-tight">{{ $pizza->name }}</h3>
                                <span class="text-base sm:text-lg font-bold text-green-600 ml-2">Rs{{ number_format($pizza->price, 2) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="inline-block bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 px-2 py-1 rounded-full text-xs font-medium">{{ $pizza->category }}</span>
                            </div>
                            @if($pizza->description)
                                <p class="text-xs sm:text-sm text-gray-600 mb-3 line-clamp-2">{{ \Illuminate\Support\Str::limit($pizza->description, 80) }}</p>
                            @endif
                            <div class="flex space-x-2">
                                <button wire:click="edit('{{ $pizza->id }}')" class="flex-1 px-3 py-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white rounded-lg hover:from-yellow-500 hover:to-yellow-600 text-xs sm:text-sm transition-all duration-200 font-medium shadow-sm">
                                    ✏️ Edit
                                </button>
                                <button wire:click="openDeleteModal('{{ $pizza->id }}')" class="flex-1 px-3 py-2 bg-gradient-to-r from-red-400 to-red-500 text-white rounded-lg hover:from-red-500 hover:to-red-600 text-xs sm:text-sm transition-all duration-200 font-medium shadow-sm">
                                    🗑️ Delete
                                </button>
                            </div>
                        </div>
        ...
        @empty
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 sm:p-12 text-center">
                <div class="text-gray-400">
                    ...
                    <p class="text-sm text-gray-500">Get started by adding your first delicious pizza! 🍕</p>
                </div>
            </div>
        @endforelse
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full table-fixed divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                    <tr>
                        <th class="w-1/5 px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Pizza</th>
                    ...
                    @forelse($pizzas as $pizza)
                        <tr class="hover:bg-gradient-to-r hover:from-gray-50 hover:to-blue-50 transition-all duration-200">
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <div class="font-semibold text-gray-900">{{ $pizza->name }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @if($editingPizzaId === $pizza->id)
                                ...
                                @else
                                    @if($pizza->image)
                                        <img src="{{ asset('storage/' . $pizza->image) }}" alt="{{ $pizza->name }}" class="w-14 h-14 object-cover rounded-lg shadow-sm">
                                    @else
                            ...
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <div class="flex flex-col space-y-2">
                                        <button wire:click="edit('{{ $pizza->id }}')" class="px-3 py-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white rounded-lg text-xs hover:from-yellow-500 hover:to-yellow-600 transition-all duration-200 font-medium">
                                            ✏️ Edit
                                        </button>
                                        <button wire:click="openDeleteModal('{{ $pizza->id }}')" class="px-3 py-2 bg-gradient-to-r from-red-400 to-red-500 text-white rounded-lg text-xs hover:from-red-500 hover:to-red-600 transition-all duration-200 font-medium">
                                            🗑️ Delete
                                        </button>
                                    </div>
                                @endif
                ...
                <div class="p-4 sm:p-6">
                    <div class="flex justify-between items-center mb-6">
                        <div class="flex items-center space-x-3">
                            ...
                            <h2 class="text-xl font-bold text-gray-900">Add New Pizza</h2>
                        </div>
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Pizza Name *</label>
                            <input wire:model="newForm.name" type="text" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" placeholder="Enter pizza name">
                            @error('newForm.name') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Pizza Image</label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input wire:model="newForm.category" type="text" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" placeholder="e.g., Vegetarian">
                        ...
                        <div>
                            ...
                            <textarea wire:model="newForm.description" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none" placeholder="Describe your delicious pizza..."></textarea>
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                <div class="p-4 sm:p-6">
                    <div class="flex items-center mb-6">
                        ...
                        <div class="ml-4">
                            <h3 class="text-lg font-bold text-gray-900">Delete Pizza</h3>
                            <p class="text-sm text-gray-600 mt-1">Are you sure you want to delete this pizza? This action cannot be undone.</p>
                        </div>
                    </div>
